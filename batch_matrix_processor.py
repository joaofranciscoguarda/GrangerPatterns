#!/usr/bin/env python3
"""
Batch Matrix Processor for Granger Causality Analysis

This script processes multiple Excel files from an input directory and generates
matrix visualizations with consistent scaling and condition-level analysis.
"""
...
async def process_matrix_analysis(input_dir="input", output_dir="output"):
    """Process files and generate matrix visualizations"""

    # Validate input directory
    if not validate_input_directory(input_dir):
        return False

    print("Granger Causality Matrix Analysis - Batch Processor")
    print("=" * 60)
    ...
    try:
        ...
        create_matrix_output_directories(output_dir)

        # Generate individual matrix visualizations
        print("\nGenerating individual matrix visualizations...")
        generate_individual_matrix_visualizations(analyzer, output_dir)

        # Generate condition-level matrix visualizations
        print("\nGenerating condition-level matrix visualizations...")
        generate_condition_level_matrix_visualizations(analyzer, output_dir)
        ...
        print(f"\n✓ Matrix analysis completed successfully!")
        print(f"✓ Results saved to: {output_dir}")
        print(f"✓ Future runs will be faster due to caching")
        return True

    except Exception as e:
        print(f"\nError during matrix processing: {str(e)}")
        import traceback
...
async def main():
    ...
    try:
        success = await process_matrix_analysis()
        if not success:
            sys.exit(1)
